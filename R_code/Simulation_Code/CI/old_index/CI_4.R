...
for(k in 1:2){
  ...
  namek = paste('CI4_', k, '.csv', sep = '') 
  namek_true = paste('CI4_', k, '_True.csv', sep = '') 
  ...
  XY_na = cbind(X_na, Y_na2) 
  
  
  XY_true_norm = sapply(XY_true, function(x) (x-min(x))/(max(x)-min(x)))
  
  XY_export = XY_true_norm
  XY_export[is.na(XY_na)] = NA 
  ...
  write.table( XY_true_norm ,  namek_true , sep = ',', col.names = F,row.names = F)
}
