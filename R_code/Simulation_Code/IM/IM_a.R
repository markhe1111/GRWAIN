...
#p1 p2 p3 are dimensions for negative, positive and complete random
p=99
## p is the wanted dimension
...
setwd ("C:\\Users\\xiaoming\\Desktop\\makr he\\GRWAIN\\Simulations\\IM_half3")
## IM, IM_negative, IM_half2, IM_half3 could be implemented by changing p1 p2 and p3

## IM_T, IM_cauchy could be change by changing distribution of Y
 ...
 for(k in 1:50){
  ...
  X_true1  = data.frame( do.call( cbind ,  lapply(multi_missings1, function(x)  x$y) ))
  M1   = data.frame( do.call( cbind ,  lapply(multi_missings1, function(x)  x$d) ))
  true_Pm1 =  data.frame( do.call( cbind ,  lapply(multi_missings1, function(x)  x$pm) ))
